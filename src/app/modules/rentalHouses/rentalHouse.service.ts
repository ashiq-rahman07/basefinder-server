...
const createRentalHouse = async (
  houseData: Partial<IRentalHouse>,
...
const getAllRentalHouse = async (query: Record<string, unknown>) => {
  const { categories, bedrooms, minPrice, maxPrice, ...pQuery } = query;
...
const getAllHouseByUser = async (req: Request) => {
  ...
  if (user.role === 'admin') {
    ...
    rentalHouses = new QueryBuilder(
      RentalHouse.find({ landlordUser: user.userId })
        .populate('landlordUser')
        .populate('category', '_id name')
        .setOptions({ strictPopulate: false }),
      query
